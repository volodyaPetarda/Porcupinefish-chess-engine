            ...
            case(1):{
                ...
                for(int j = 0; j < n; j++) {
                    ...
                    for (int i = 0; i < train_cnt; i++) {
                        ...
                        std::pair<float, bool> res = nn.train(pixels_train[i % train_cnt], labels_train[i % train_cnt], alpha);
                        error += res.first;
                    }
                    error /= train_cnt;
...
//                    network->setErrorActivationType(ActivationType::CustomError);
                    for(int j = 0; j < randomMoves; j++){
                        ...
                        if(!parentBoard.isPossibleMove(move))
                            continue;
                        if(move[0] == tempData[i - 1][0] && move[1] == tempData[i - 1][1] && move[2] == tempData[i - 1][2] && move[3] == tempData[i - 1][3])
                            continue;
...
//                        const int TEST_COUNT = 1000000;
                ...
                for(int j = 0; j < correctCount.size(); j++){
                    std::cout<<j<<") "<<correctCount[j] / movesCount[j]<<' '<<sum[j] / movesCount[j]<<' '<<movesCount[j]<<'\n';
                }
                break;
            }
            case(2):{
    ...
    while(true) {
        ...
        switch (op) {
            case (1): {
                ...
                for (int q = 1; q <= n; q++) {
//                    const int N = 5000, TEST_COUNT = 2000;
                    const int N = 500, TEST_COUNT = 200;
//                    if(q > 0 && q % N == 0){
                    if(q % N == 0){
                ...
                for(int i = 0; i < n; i++){
                    ...
                    for(int j = 0; j < tempData.size(); j++){
                        ...
                        for(int k = 0; k < UNDER; k++){
                            ...
                            if(resUci[0] == tempData[j][0] && resUci[1] == tempData[j][1] && resUci[2] == tempData[j][2] && resUci[3] == tempData[j][3]){
            ...
            case(7):{
                ...
                for(int i = 0; i < n; i++){
                    ...
                    for(int j = 0; j < tempData.size(); j++){
                        ...
                        if(move[0] == tempData[j][0] && move[1] == tempData[j][1] && move[2] == tempData[j][2] && move[3] == tempData[j][3]) {
                            result++;
            ...
            case(8):{
                ...
                while(true){
                    ...
                    if(!isWhite) {
                        ...
                        std::string resUci = predictor->predictMove(board, 2, 0.05f / sqrtf(ind));
                        cout<<"moves: "<<resUci<<'\n';
                        board.move(resUci);
                    }
                    else{
